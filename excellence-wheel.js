...
(function (root, factory) {
  ...
  function create(container, options) {
    ...
    var selectedId = null;
    var hoveredId = null;
    // Wedges picked out by setHighlighted() (e.g. the app's school
    // priorities). They only show while nothing is selected: they lift
    // like a selected wedge, and every other wedge dims.
    var highlighted = {};
    function highlightActive() {
      return selectedId === null && Object.keys(highlighted).length > 0;
    }
    ...
    function updateRingHighlights() {
      ...
      if (highlightActive()) {
        Object.keys(highlighted).forEach(function (id) { highlightedCategories[segmentById(id).category] = true; });
      }
      var anyActive = hoveredCategory !== null || selectedCategory !== null || highlightActive();
      rings.forEach(function (ring) {
        var cat = ring.getAttribute('data-category');
        var isActive = cat === hoveredCategory || cat === selectedCategory || !!highlightedCategories[cat];
    ...
    // hovered elsewhere. Highlighted wedges (see setHighlighted()) count
    // as active too, while nothing is selected.
    function updateWedgeDimming() {
      var showHighlight = highlightActive();
      var anyActive = hoveredId !== null || selectedId !== null || showHighlight;
      wedges.forEach(function (el) {
        var id = el.getAttribute('data-id');
        var isActive = id === hoveredId || id === selectedId || (showHighlight && !!highlighted[id]);
    ...
    function applySelection() {
      ...
      wedges.forEach(function (el) {
        ...
        el.classList.toggle('is-highlighted', showHighlight && !!highlighted[el.getAttribute('data-id')]);
    ...
    return {
      ...
      // Picks out these wedge ids while nothing is selected ([] clears).
      setHighlighted: function (ids) {
        highlighted = {};
        (ids || []).forEach(function (id) { if (segmentById(id)) highlighted[id] = true; });
        applySelection();
        updateRingHighlights();
        updateWedgeDimming();
      },
  ...
  return {
    ...
    // A plain circular arc at radius r (for lines along a slice).
    arcLinePath: function (r, startDeg, endDeg) {
